...
- TOKEN-FIRST: /erc20/:token/swaps (Moralis)
- Nessun filtro su DEX o pair: accettiamo i BUY del token su qualunque pool
- DexScreener usato per: main pair (solo per link), prezzo di riferimento e attività aggregata
...
def date_to_block_cached(dt_iso: str) -> int:
    ...
    for f in ("block","block_number","blockNumber"):
        if f in j:
            block = int(j[f]); CACHE.set("date_to_block", k, block); return block
...
def get_token_metadata_cached(token: str) -> Tuple[str, int]:
    ...
    CACHE.set("metadata", token, [sym, dec]); return sym, dec
...
def get_token_price_usd_moralis_cached(token: str) -> Optional[float]:
    ...
    CACHE.set("price_moralis", token, price); return price
...
def dexscreener_pick_pair_and_activity(token: str, minutes: int):
    ...
    for p in pairs:
        ...
        for b in ("m5","h1","h6","h24"):
            t = txns.get(b) or {}
            try: agg_buys[b] += int(t.get("buys") or 0)
            except: pass
            try: agg_sells[b] += int(t.get("sells") or 0)
            except: pass
    ...
    log.info(f"DexScreener main pair: {pair} | price={price} | {bucket} buys={buys_b} sells={sells_b} | liq=${best_liq}")
...
def get_wallet_first_tx_ts_cached(addr: str) -> Optional[int]:
    ...
    CACHE.set("first_tx_ts", addr, ts_out); return ts_out
...
def parse_args():
    ap = argparse.ArgumentParser(description="Young Whale Finder (BSC; Moralis-min; DexScreener-aggregated; TOKEN-FIRST)")
...
def main():
    ...
    if price_usd is None:
        try:
            ...
            log.warning(str(e)); price_usd = None; price_source = "none"
    ...
    # Limiti dinamici basati sull'attività aggregata coerente con --minutes
    TK = TOP_K
    MAX = MAX_SWAPS
    if isinstance(agg_buys, int) and agg_buys >= 0:
        # euristica: stima per minuto
        est_per_min = max(1, int(round(agg_buys / max(1, minutes))))
        ...
        if dyn_cap < MAX or dyn_topk < TK:
            log.info(f"Dynamic limits: MAX_SWAPS {MAX}→{dyn_cap}, TOP_K {TK}→{dyn_topk} (agg_buys={agg_buys})")
            MAX = dyn_cap; TK = dyn_topk

    buyers_sum_usd: Dict[str, float] = {}
    total_swaps = total_swaps_in_window = total_buys_ge_min = 0
    c_not_buy = c_below_min = c_no_wallet = 0
    ...
    log.info(f"Swaps scanned={total_swaps} | in_window={total_swaps_in_window} | buys≥min={total_buys_ge_min} | wallets={len(buyers_sum_usd)}")

    # Aggregati per recap
    buyers_total_usd_all = float(sum(buyers_sum_usd.values()))
    buyers_unique_all = int(len(buyers_sum_usd))

    # TOP_K per valutazione filtri advanced (young+balance)
    ...
    out = {
        ...
        "from_iso": from_iso,
        "to_iso": to_iso,
        "main_pair_hint": main_pair,  # solo per link
        "stats": {
