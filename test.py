...
# -*- coding: utf-8 -*-
"""
Crypto Screener (v2.1) — 3 criteri + funnellizzazione
...
- Stage-gating aggressivo (stop prima delle call care).
- Cache multilivello (perps, CoinGecko per coin, DexScreener).
- Indice locale state/index.json per saltare coin stabili tra i run.
- Euristiche Bitget (short-circuit sui primi N tickers e dominanza volume).
- Parallelismo controllato (--workers) con rate-limit per sorgente.
- Report “a imbuto” per contare e LISTARE chi passa ogni stage.

Requisiti: requests, tenacity, rich
  pip install requests tenacity rich

  python test.py --pages 2 --per-page 250 --workers 10 --min-liq 200000 --max-tickers-scan 25 --dominance 0.35 --skip-unchanged-days 3 --rps-cg 0.6 --rps-ds 2.0 --funnel-show 30
...
class Settings:
    ...
    # Funnel printing
    funnel_show: int = 30    # quante coin mostrare per stage
...
def top_spot_cex_bitget_ok(coin: Dict[str, Any], dominance: float, max_scan: int) -> Tuple[bool, Optional[str]]:
    """Ritorna (is_bitget_top, top_name) con short-circuit euristico basato su dominanza."""
...
def screen_coin(cid: str, sym_hint: str,
    ...
    # ---- Fetch CoinGecko solo se passa Stage 1 ----
    try:
        coin = fetch_coin_full(cid, cg_rl)
        fr.name = coin.get("name") or cid
    except Exception as e:
        # se CG fail, non possiamo valutare stage 2/3
    ...
    reasons_ok.append("LP principale Pancake")

    # aggiorna state persistente (fine Stage 3)
    state[cid] = {
        "last_checked": now_ts(),
        "status": "done",
...
def _print_stage_list(title: str, rows: List[FunnelRow], show: int, extra_cols: Optional[List[Tuple[str, str]]] = None):
    """extra_cols: list of (header, attr_name) to include."""
    if not rows:
        if RICH: console.print(f"[bold yellow]{title}: 0[/bold yellow]")
        else: print(f"{title}: 0")
        return
    if RICH:
        t = Table(title=f"{title} — {len(rows)}")
...
def print_funnel(funnel_rows: List[FunnelRow], show: int, total_candidates: int):
    ...
    if RICH:
        console.rule("[bold]Funnel (run corrente)[/bold]")
        console.print(f"Totale candidati: [bold]{total_candidates}[/bold] | Processati: [bold]{len(processed)}[/bold] | Skippati: [bold]{total_skipped}[/bold] (recenti)")
    else:
        print("==== Funnel ====")
    ...
    # Mostra anche dove Bitget è top, con colonna 'top_spot_cex'
...
def print_results(results: List[CheckResult], funnel_rows: List[FunnelRow], total_candidates: int):
    ...
    # Output finale “coerente” con 3 criteri
    all3 = [r for r in results if r.score() >= 2 and r.ok()]  # 2 OK: Bitget + Pancake (BSC) (Perps è gate fuori dal Result)
    at_least2 = []
    # almeno 2 = chi ha passato S1+S2 (anche se S3 no). Usiamo funnel_rows.
    ...
    if RICH:
        if all3:
            t = Table(title="Coin che rispettano TUTTI e 3 i parametri")
            t.add_column("Symbol"); t.add_column("Name"); t.add_column("Top Spot CEX"); t.add_column("LP (Dex)"); t.add_column("LP Liquidity $")
            for r in all3:
        ...
        if at_least2:
            t = Table(title="Coin che rispettano ALMENO 2 parametri (S1+S2)")
...
def parse_args() -> Settings:
    ap = argparse.ArgumentParser(description="Crypto screener per CEX/DEX (3 criteri + funnel)")
    ...
    # --- compat flags non usati in questa versione (lasciati per compatibilità) ---
    ...
    ap.add_argument("--require-v3", action="store_true", help=argparse.SUPPRESS)
