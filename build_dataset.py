# build_dataset.py
# Dataset builder per selezionare pool simili su PancakeSwap (v2+v3) su BSC
# e scaricare OHLCV 5m da GeckoTerminal. Include fallback robusti.
# NOVITÀ:
# - Supporto Pancake v3 (pancakeswapv3 / pancakeswap-v3)
# - CLI: --dex_ids per scegliere i dexIds; --allow_any_dex per ignorare il filtro DEX
...
DEFAULT_HEADERS = {
    ...
    "user-agent": "dex-forecaster/2.1 (+cli)",
...
def cache_get(url: str, params: Optional[Dict[str, Any]] = None,
              headers: Optional[Dict[str, str]] = None, cache_dir: str = "data/cache",
...
def _vol_h24(p: Dict[str, Any]) -> float:
    v = p.get("volume")
    if isinstance(v, dict): return _safe_float(v.get("h24"), 0.0)
...
def _txns_h24(p: Dict[str, Any]) -> int:
    ...
    if isinstance(t, dict):
        h24 = t.get("h24")
        if isinstance(h24, dict): return _safe_int(h24.get("buys"),0)+_safe_int(h24.get("sells"),0)
        if isinstance(h24,(int,float)): return _safe_int(h24,0)
    for k in ("txns24h","transactions24h","txns24","transactions"):
        v = p.get(k)
        if isinstance(v, dict): return _safe_int(v.get("buys"),0)+_safe_int(v.get("sells"),0)
        if v is not None: return _safe_int(v,0)
    return 0

def _liq_usd(p: Dict[str, Any]) -> float:
    liq = p.get("liquidity")
    if isinstance(liq, dict): return _safe_float(liq.get("usd"), 0.0)
...
def ds_get_pair(chain: str = NETWORK, pair_addr: str = "") -> Dict[str, Any]:
    ...
    if not pairs: raise ValueError(f"Nessun pair trovato per {chain}/{pair_addr}.")
    return pairs[0]
...
def ds_pairs_by_dex(chain: str = NETWORK, dex_ids: Tuple[str, ...] = DEFAULT_PCS_DEX_IDS) -> List[Dict[str, Any]]:
...
def _similarity_score(p: Dict[str, Any], tgt: Dict[str, Any]) -> float:
    ...
    s = 1.5*logdist(liq, t_l or 1.0) + 1.2*logdist(mcap or 1.0, t_m or 1.0) + 1.0*logdist(vol or 1.0, t_v or 1.0) + 0.8*logdist(tx or 1.0, t_t or 1.0)
    if age is not None and t_age is not None and t_age > 0: s += 0.2*abs((age - t_age)/max(t_age,1e-6))
    return s

def _apply_relax(filters: Dict[str, float], relax: float) -> Dict[str, float]:
    if relax == 1.0: return filters
    f = dict(filters)
    f["liquidity_min"] /= relax; f["liquidity_max"] *= relax
    f["mcap_min"]      /= relax; f["mcap_max"]      *= relax
    f["vol24_min"]     /= relax; f["vol24_max"]     *= relax
    f["tx24_min"]       = int(f["tx24_min"]/relax); f["tx24_max"] = int(f["tx24_max"]*relax)
    f["age_days_min"]  /= relax; f["age_days_max"]  *= relax
    return f

def _adaptive_filters_around_target(tgt: Dict[str, Any]) -> Dict[str, float]:
    liq = max(_liq_usd(tgt), 1.0); mcap = max(_mcap_or_fdv(tgt), 1.0); vol = max(_vol_h24(tgt), 1.0); tx  = max(_txns_h24(tgt), 1)
    age = _age_days_from_ms(tgt.get("pairCreatedAt")) or 7.0
    def band(val, low_mult, high_mult, low_floor, high_cap):
        lo = max(val * low_mult, low_floor); hi = min(val * high_mult, high_cap)
...
def _filters_failures(
    ...
    allowed_dex_ids: Optional[set],   # None -> nessun filtro DEX
) -> Dict[str, int]:
    fails: Dict[str, int] = {}

    # Filtro DEX (se specificato)
    if allowed_dex_ids is not None:
        d = (p.get("dexId") or "").lower()
        if d not in allowed_dex_ids:
    ...
    if quote_mode != "off":
        ...
        if quote_mode == "single":
            if not quote_single: fails["quote_single_not_set"] = 1
            elif q != quote_single: fails["quote!=selected"] = 1
        elif quote_mode == "allow":
            if q not in quote_allow_set: fails["quote_not_in_allowlist"] = 1
...
def select_similar(
    ...
    _tgt_obj: Optional[Dict[str, Any]] = None,
    dex_ids: Optional[List[str]] = None,          # lista dexIds consentiti per gli endpoint "by dex"
    allow_any_dex: bool = False,                  # True = nessun filtro DEX sui candidati
) -> pd.DataFrame:
    ...
    # Filtro DEX consentiti (None = no filter)
    allowed_dex_ids = None if allow_any_dex else set([d.lower() for d in (dex_ids or list(DEFAULT_PCS_DEX_IDS))])
    ...
    anchors_use = [a.lower() for a in (anchors if anchors else (DEFAULT_ANCHORS if source != "wbnb-only" else [WBNB]))]
    for a in anchors_use:
        try:
            cands.extend(ds_token_pairs(a, chain=chain))
        except Exception:
            if explain: print(f"[warn] anchor token-pairs fallita per {a}")

    if source in ("both", "dex-only"):
        try:
            cands.extend(ds_pairs_by_dex(chain, tuple(dex_ids or list(DEFAULT_PCS_DEX_IDS))))
        except Exception:
            if explain: print("[warn] fetch pairs_by_dex fallita")
    ...
    for p in cands:
        ...
        if not addr or addr in seen or addr == target_pair_addr.lower(): continue
        seen.add(addr)
        if hydrate_missing: p = hydrate_candidate_if_needed(p, chain=chain)
        uniq.append(p)

    if explain: print(f"[debug] candidati totali (dedup): {len(uniq)}")

    if diag and uniq:
        from collections import Counter
        dex_ct = Counter([(p.get("dexId") or "").lower() for p in uniq]).most_common(8)
    ...
    for p in uniq:
        fails = _filters_failures(
            p, hf, quote_mode, quote_single, quote_allow_set,
        ...
        if not fails:
            ...
            if explain:
                for k in fails: agg_reasons[k] = agg_reasons.get(k, 0) + 1

    if not rows:
        if explain:
            print("[explain] conteggio motivi di esclusione:")
            for k, v in sorted(agg_reasons.items(), key=lambda x: -x[1]): print(f"  - {k}: {v}")
            print("[hint] Prova --allow_any_dex oppure amplia --dex_ids, usa --no_dedup_base e/o --relax più alto.")
    ...
    if dedup_base:
        ...
        for r in rows:
            b = r["base_address"]
            if b and b not in best_by_base: best_by_base[b] = r
...
def save_parquet_or_csv(df: pd.DataFrame, out_parquet: str) -> Optional[str]:
    ...
    try:
        if engine:
            df.to_parquet(out_parquet, index=False, engine=engine); return out_parquet
...
def gt_fetch_ohlcv_5m(pool: str, network: str = NETWORK, days: int = 30,
                      include_empty: bool = True, cache_dir: str = "data/cache") -> pd.DataFrame:
    end = int(datetime.now(tz=timezone.utc).timestamp()); start = end - days*86400
    url = f"{GT_API}/networks/{network}/pools/{pool}/ohlcv/minute"
    out: List[Tuple[int, float, float, float, float, float]] = []; before: Optional[int] = None; agg = 5
    guard_max_rows = int(days*288 + 3000)

    while True:
        params = {"aggregate": agg, "limit": 1000, "currency": "usd", "include_empty_intervals": str(include_empty).lower()}
        ...
        for row in lst_sorted:
            try:
                ts,o,h,l,c,v = row
                if start <= ts <= end: out.append((int(ts), float(o), float(h), float(l), float(c), float(v)))
            except Exception:
                continue
    ...
    if not include_empty:
        ...
        df = (df.set_index("ts").reindex(full).ffill().fillna({"volume":0.0}).rename_axis("ts").reset_index())
...
def try_select_with_fallbacks(
    target_pair: str, count: int, network: str, filters: Dict[str, float], source: str,
    quote_addr: str, quotes_allow: Optional[List[str]], no_quote_filter: bool,
    anchors: Optional[List[str]], diag: bool, dedup_base: bool, explain: bool,
    auto_fallback: bool, adaptive_filters: bool,
    ...
    try:
        return select_similar(
            ...
            dex_ids=dex_ids, allow_any_dex=allow_any_dex
        )
    except Exception:
        if not auto_fallback: raise
...
# -------------------- Orchestrator --------------------
def build_dataset(
    target_pair: str, count: int = 50, days: int = 30, outdir: str = "data/gold",
    db: str = "data/predictor.db", cache_dir: str = "data/cache", network: str = NETWORK,
    save_list_path: str = "data/pools_pancake_list.csv", filters: Optional[Dict[str, float]] = None,
    explain: bool = False, relax: float = 1.0, source: str = "both", quote_addr: str = WBNB,
    quotes_allow: Optional[List[str]] = None, no_quote_filter: bool = False, anchors: Optional[List[str]] = None,
    diag: bool = False, dedup_base: bool = True, auto_fallback: bool = True, adaptive_filters: bool = True,
    dex_ids: Optional[List[str]] = None, allow_any_dex: bool = False
) -> Tuple[pd.DataFrame, List[str]]:
    os.makedirs(outdir, exist_ok=True); os.makedirs(os.path.dirname(save_list_path), exist_ok=True)
    filters = filters or DEFAULT_FILTERS

    print("[1/4] Selezione PCS (v2+v3) su BSC con filtri hard...")
    ...
    for i, row in enumerate(df_pools.itertuples(index=False), start=1):
        ...
        if os.path.exists(out_parquet) or os.path.exists(out_parquet.replace(".parquet",".csv.gz")) or os.path.exists(out_parquet.replace(".parquet",".csv")):
            print(f"  [{i}/{n}] {pair}  (skip: già presente)"); continue
        try:
            print(f"  [{i}/{n}] {pair}  ... fetching")
            df = gt_fetch_ohlcv_5m(pair, network=network, days=days, include_empty=True, cache_dir=cache_dir)
            saved = save_parquet_or_csv(df, out_parquet); append_sqlite(df, network, pair, db_path=db)
            print(f"       -> rows={len(df)} saved={saved}")
            if saved: saved_files.append(saved)
        except Exception as e:
            print(f"       !! errore su {pair}: {e}")
            if os.environ.get("DEBUG_TRACE","0") == "1": traceback.print_exc()
...
# -------------------- CLI --------------------
def parse_args():
    ap = argparse.ArgumentParser(description="Dataset PCS (v2+v3) su BSC simili al target (OHLCV 5m) con fallback robusti.")
    ap.add_argument("--target_pair", required=True)
    ...
    ap.add_argument("--quote_addr", default=WBNB)
    ...
    ap.add_argument("--no_dedup_base", action="store_true")
    ap.add_argument("--no_auto_fallback", action="store_true")
    ap.add_argument("--no_adaptive_filters", action="store_true")

    # NOVITÀ: DEX controls
    ap.add_argument("--dex_ids", default=",".join(DEFAULT_PCS_DEX_IDS),
                    help="Lista dexIds separati da virgola (es: pancakeswap,pancakeswapv2,pancakeswapv3).")
    ap.add_argument("--allow_any_dex", action="store_true",
                    help="Ignora completamente il filtro DEX (accetta qualsiasi dexId).")
...
def main():
    ...
    try:
        pools_df, saved = build_dataset(
            ...
            count=args.count, days=args.days, outdir=args.outdir, db=args.db, cache_dir=args.cache_dir,
            network=NETWORK, save_list_path=os.path.join("data","pools_pancake_list.csv"),
            filters=filters, explain=args.explain, relax=args.relax, source=args.source,
            quote_addr=(args.quote_addr or "").lower(), quotes_allow=quotes_allow,
            no_quote_filter=args.no_quote_filter, anchors=anchors, diag=args.diag,
            dedup_base=not args.no_dedup_base, auto_fallback=not args.no_auto_fallback,
            adaptive_filters=not args.no_adaptive_filters, dex_ids=dex_ids,
            allow_any_dex=bool(args.allow_any_dex),
        )
        print("\n=== POOLS SELEZIONATI (top 10) ===")
        try: print(pools_df.head(10).to_string(index=False))
        except Exception: print(pools_df.head(10))
    except Exception as e:
