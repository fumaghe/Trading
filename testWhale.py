...
- Finestra temporale passata a Moralis (fromDate/toDate in ISO) per NON bruciare CU
...
Uso:
  # finestra relativa (minutes)
  python test.py --token 0x... --minutes 5 --min-usd 500 --young-days 8

  # finestra assoluta (ISO UTC)
  python test.py --token 0x... \
    --from-iso 2025-11-06T01:00:00Z \
    --to-iso   2025-11-06T01:30:00Z \
    --min-usd 600 \
    --young-days 12
"""
...
# Defaults
YOUNG_DAYS_DEFAULT = int(os.getenv("YWF_YOUNG_DAYS", "8"))
RPS = float(os.getenv("YWF_RPS", "2"))
HTTP_TIMEOUT = 25
UA = "Mozilla/5.0 (YoungWhaleFinder/1.10-STABLE; +https://moralis.com)"
...
def get_or_assign_name(addr: str, names: List[str]) -> str:
    ...
    used = { (v.get("value") if isinstance(v, dict) else v) for v in raw.values() }
...
def iterate_swaps_token_first(token: str, main_pair: Optional[str],
                              from_ts: int, to_ts: int, from_iso: str, to_iso: str):
    """
    Ora chiediamo direttamente a Moralis la finestra desiderata.
    Usiamo fromDate/toDate in **ISO 8601** (come raccomandato), più transactionTypes=buy.
    """
    cursor = None
    page = 0
    ...
    while True:
        params = {
            ...
            # filtro server-side per NON sprecare CU: ISO 8601
            "fromDate": from_iso,
            "toDate":   to_iso,
        }
        if cursor:
        ...
        if not result:
            ...
            if consecutive_old_pages >= 2:
                log.info("Early-stop: 2 pagine consecutive tutte fuori finestra.")
                return
            continue
...
def parse_args():
    ...
    ap.add_argument("--minutes", type=int, default=5, help="Finestra in minuti (se non usi --from-iso/--to-iso)")
    ...
    ap.add_argument("--from-iso", type=str, default=None, help="Inizio finestra ISO (es. 2025-11-06T01:00:00Z)")
    ap.add_argument("--to-iso",   type=str, default=None, help="Fine finestra ISO (es. 2025-11-06T01:30:00Z)")
...
def main():
    ...
    if args.from_iso and args.to_iso:
        from_dt = _parse_iso_to_utc_dt(args.from_iso)
        to_dt   = _parse_iso_to_utc_dt(args.to_iso)
        if to_dt <= from_dt:
            raise ValueError("--to-iso deve essere > --from-iso")
    else:
        from_dt = now - timedelta(minutes=minutes)
        to_dt   = now

    # Applico lo slack direttamente alla fine-finestra PRIMA di creare la ISO per Moralis
    to_dt_with_slack = to_dt + timedelta(seconds=WINDOW_SLACK_SEC)

    from_iso = from_dt.isoformat().replace("+00:00", "Z")
    to_iso   = to_dt_with_slack.isoformat().replace("+00:00", "Z")
    from_ts  = int(from_dt.timestamp())
    to_ts    = int(to_dt_with_slack.timestamp())

    window_minutes_real = max(1, int(round((to_ts - from_ts)/60)))

    log.info(f"Start | token={token} minutes={window_minutes_real} min_usd={min_usd} young_days={YOUNG_DAYS} | TOP_K={TOP_K} MAX_SWAPS={MAX_SWAPS} budget_calls={BUDGET_CALLS} log={LOG_LEVEL.lower()}")
    ...
    # Limiti dinamici (meno importanti con filtro server-side)
    TK = TOP_K
    MAX = MAX_SWAPS
    if m5_buys is not None and m5_buys >= 0:
        est_per_min = max(1, int(round(m5_buys / 5)))
        win_mins = max(1, int((to_ts - from_ts)/60))
        dyn_cap = max(200, min(MAX_SWAPS, est_per_min * win_mins * 60))
        dyn_topk = max(8, min(TOP_K, 5 + est_per_min * win_mins))
    ...
    total_swaps = total_swaps_pancake = total_tx_ge_min = 0
    ...
    log.info(f"Swaps scanned={total_swaps} | pancake={total_swaps_pancake} | tx≥min={total_tx_ge_min} | wallets={len(buyers_sum_usd)}")
    ...
    young_cutoff_ts = now_ts - YOUNG_DAYS * 86400
    results: List[dict] = []
    kept = skipped_old = 0
    ...
    out = {
        ...
        "window_minutes": window_minutes_real,
        "min_usd": min_usd,
