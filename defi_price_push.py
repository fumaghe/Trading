...
Usage
-----
python defi_price_push.py --pool 0xYourPool --rpc https://bsc-dataseed.binance.org --plot
...
def analyze_pool(pool_addr: str, rpc_url: str, factors: Optional[List[Decimal]] = None) -> Dict:
    """
    Libreria: ritorna un dict con snapshot sintetico e i risultati per i fattori richiesti.
    Non stampa nulla.
    """
    factors = factors or [Decimal('1.3'), Decimal('1.5'), Decimal('2.0')]
    ...
    pool_addr_cs = Web3.to_checksum_address(pool_addr)
    pool = w3.eth.contract(address=pool_addr_cs, abi=UNISWAP_V3_POOL_MINI_ABI)

    snapshot = read_pool_snapshot(w3, pool_addr_cs)
    P0, rows = compute_targets(snapshot, w3, pool, factors=factors)

    return {
        "pool": pool_addr.lower(),
        "price0": f"{P0:.18f}",
        "token0": {"symbol": snapshot.token0.symbol, "decimals": snapshot.token0.decimals, "address": snapshot.token0.address},
        "token1": {"symbol": snapshot.token1.symbol, "decimals": snapshot.token1.decimals, "address": snapshot.token1.address},
        "tick": snapshot.tick,
        "tickSpacing": snapshot.tickSpacing,
        "liquidity": snapshot.liquidity,
        "block": {"number": snapshot.block_number, "timestamp": snapshot.block_timestamp},
        "targets": rows,  # ciascun item ha: target, price_target, tick_target, token1_needed, segments, ...
    }
...
def compute_targets(
    ...
    w3: Web3,
    pool,
    factors: List[Decimal] = [Decimal('1.5'), Decimal('2.0'), Decimal('4.0')],
):
    results = []
