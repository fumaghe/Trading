...
# -*- coding: utf-8 -*-
"""
Crypto Screener (v2.2) — 3 criteri + funnel, cache-aware & robust
...
Ottimizzazioni / Robustezza:
- Stage-gating (perps -> CG Bitget -> DexScreener) con cache multilivello.
- Rate limiter con leggero jitter per ridurre 429.
- Filtri tickers CoinGecko: skip "stale/anomaly" e trust_score non verde/giallo.
- Stato persistente thread-safe (lock) per evitare write races.
- Mini “retry pool” a fine run solo per i cg_error, con RPS ridotto.
- Report funnel (S1, S1+S2, S1+S2+S3) e stampa riassuntiva finale.
...
class RateLimiter:
    ...
    def wait(self):
        with self.lock:
            ...
            self.last = time.time()
        # aggiungi jitter brevissimo per desincronizzare chiamate concorrenti
...
def top_spot_cex_bitget_ok(coin: Dict[str, Any], dominance: float, max_scan: int) -> Tuple[bool, Optional[str]]:
    ...
    for t in tickers:
        ...
        if name.lower().startswith("bitget"):
            bitget_vol += vol
        # short-circuit sui primi N tickers
...
def parse_args() -> Settings:
    ap = argparse.ArgumentParser(description="Crypto screener per CEX/DEX (3 criteri + funnel, cache-aware & robust)")
...
# --------------------------- Main --------------------------- #
# python test2.py --pages 8 --per-page 250 --workers 12 --min-liq 200000 --max-tickers-scan 40 --dominance 0.30 --skip-unchanged-days 0 --rps-cg 0.5 --rps-ds 2.0 --funnel-show 100
