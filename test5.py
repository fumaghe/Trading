...
# -*- coding: utf-8 -*-
"""
Crypto Screener (v2.3.3) — Rich logs + TOP-only output finale

Novità vs v2.3.2:
- NIENTE "quiet" forzato: i log e le tabelle Rich compaiono se NON usi --quiet.
- Progress bar Rich durante lo screening.
- Pannelli funnel per ogni stage (S1/S2/S3) con conteggi e anteprime (fino a --funnel-show).
- Persistenza reale dello stato in 'state/index.json' (funzionano gli skip tra run).
- Messaggi di avviso/errore ben formattati (Rich).

Criteri invariati (coin entra nel TOP):
  1) Perpetual su ENTRAMBI: Binance Futures (USDT-M) e Bybit (linear)
  2) CEX spot principale per volume = Bitget (euristica dai tickers CoinGecko)
  3) Contratto su BSC e main LP (maggiore liquidità) su PancakeSwap
     (opzione --require-v3 per limitare solo a Pancake v3)

Esempio:
python test5.py --seed-from perps --workers 12 --min-liq 200000 --max-tickers-scan 40 --dominance 0.30 --skip-unchanged-days 0 --rps-cg 0.5 --rps-ds 2.0 --funnel-show 100
"""
...
def load_state() -> Dict[str, Any]:
    ...
    try:
        ...
        warn(f"Impossibile leggere lo state: {e}")
        return {}
...
def save_state(state: Dict[str, Any]):
    try:
        ...
        warn(f"Impossibile salvare lo state: {e}")

def stale_by_days(ts: Optional[int], days: int) -> bool:
    # Se days <= 0 => rielabora sempre (niente skip)
    if days <= 0:
        return True
    if not ts:
        return True
...
def screen_coin(cid: str, sym_hint: str,
    ...
    try:
        ...
        if settings.verbose and not settings.quiet:
            warn(f"CoinGecko error per {cid}: {e}")
        with state_lock:
    ...
    try:
        ...
        pair = None
        if settings.verbose and not settings.quiet:
            warn(f"DexScreener error per {cid}: {e}")
    if not pair:
        with state_lock:
...
def build_seed_from_perps(cg_rl: RateLimiter,
    ...
    if RICH and not settings.quiet:
        ...
        if unmapped[:20]:
            console.print(Panel.fit(
                ", ".join(unmapped[:20]) + (f" ... (+{len(unmapped)-20})" if len(unmapped) > 20 else ""),
...
def render_config_panel():
    if settings.quiet or not RICH or not console:
        return
    body = (
        f"[b]Seed:[/b] {settings.seed_from}\n"
        f"[b]Workers:[/b] {settings.workers}\n"
        f"[b]Min LP:[/b] {int(settings.min_pair_liquidity_usd)} USD"
        f"    [b]Require v3:[/b] {settings.require_v3}\n"
        f"[b]Bitget dominance:[/b] {settings.bitget_dominance:.2f}"
        f"    [b]Max tickers scan:[/b] {settings.max_tickers_scan}\n"
        f"[b]RPS CG:[/b] {settings.rps_cg}    [b]RPS DS:[/b] {settings.rps_ds}\n"
        f"[b]Skip unchanged days:[/b] {settings.skip_unchanged_days}\n"
        f"[b]Cache:[/b] {settings.cache_root}    [b]State:[/b] {settings.state_root}\n"
    )
    console.print(Panel(body, title="Configurazione", border_style="magenta"))

def render_funnel_tables(funnel_rows: List[FunnelRow], results: List[CheckResult]):
    if settings.quiet or not RICH or not console:
        return
    total = len(funnel_rows)
    s1 = [fr for fr in funnel_rows if fr.s1_perps]
    s2 = [fr for fr in funnel_rows if fr.s1_perps and fr.s2_bitget]
    s3 = [fr for fr in funnel_rows if fr.s1_perps and fr.s2_bitget and fr.s3_bsc_pancake]
    skipped = [fr for fr in funnel_rows if fr.skipped]
    no_perps = [fr for fr in funnel_rows if (not fr.s1_perps) and (not fr.skipped)]
    not_bitget = [fr for fr in funnel_rows if fr.s1_perps and (not fr.s2_bitget)]
    no_pancake = [fr for fr in funnel_rows if fr.s1_perps and fr.s2_bitget and (not fr.s3_bsc_pancake)]

    console.print(Rule("[bold]Funnel[/bold]"))
    console.print(Panel.fit(
        f"Totali: [bold]{total}[/bold]  |  S1 pass: [bold]{len(s1)}[/bold]  |  S2 pass: [bold]{len(s2)}[/bold]  |  S3 pass: [bold]{len(s3)}[/bold]\n"
        f"Skipped (recent): [bold]{len(skipped)}[/bold]",
        border_style="cyan", title="Riepilogo"))

    def table_preview(title: str, rows: List[FunnelRow], kind: str):
        if not rows:
            return
        ...
        if kind == "S1":
            ...
            t.add_column("Symbol", style="bold")
            t.add_column("Name")
            t.add_column("Top spot CEX")
        elif kind == "S3":
            t.add_column("Symbol", style="bold")
            t.add_column("Name")
            t.add_column("LP USD", justify="right")
            t.add_column("Pool (short)")
        else:
            t.add_column("Symbol", style="bold")
            t.add_column("Reason/Top/Note")

        limit = min(len(rows), settings.funnel_show)
        for fr in rows[:limit]:
            if kind == "S1":
                t.add_row(fr.symbol or "", fr.name)
            elif kind == "S2":
                t.add_row(fr.symbol or "", fr.name, fr.top_spot_cex or "-")
            elif kind == "S3":
                liq = "-"
                pool = "-"
                if fr.lp_pair:
                    liq = format_eur_style_usd(fr.lp_pair.get("liquidityUsd"))
                    pool = (fr.lp_pair.get("pairAddress") or "")
                    # abbreviato solo nella tabella
                    if len(pool) > 14:
                        pool = pool[:10] + "..." + pool[-4:]
                t.add_row(fr.symbol or "", fr.name, liq, pool)
            else:
                note = fr.reason or fr.top_spot_cex or "-"
                t.add_row(fr.symbol or "", note)
        console.print(t)

    # anteprime
    table_preview("S1  • Perps su entrambi (pass)", s1, "S1")
    table_preview("S2  • Top spot CEX = Bitget (pass)", s2, "S2")
    table_preview("S3  • BSC + Pancake LP (pass)", s3, "S3")

    if no_perps:
        table_preview("KO • No perps su entrambi", no_perps, "KO")
    if not_bitget:
        table_preview("KO • Top CEX non Bitget", not_bitget, "KO")
    if no_pancake:
        table_preview("KO • Niente Pancake / LP bassa", no_pancake, "KO")
    if skipped:
        table_preview("SKIPPED • Recent check", skipped, "SKIP")
...
def main():
    ...
    # Header e config
    if not settings.quiet and RICH and console:
        console.print(Rule("[bold]Crypto Screener — Rich mode[/bold]"))
        render_config_panel()
    ...
    try:
        ...
        err(f"Errore caricando basi perps: {e}")
        sys.exit(1)
    ...
    # Log funnel + riepilogo (se non quiet)
    if not settings.quiet:
        render_funnel_tables(funnel_rows, results)
        if RICH and console:
            ok_count = len([r for r in results if r.score() >= 2 and r.ok()])
            console.print(Panel.fit(
                f"[bold green]TOP trovati:[/bold green] {ok_count}   •   "
                f"Universe analizzato: {len(funnel_rows)}",
                title="Esito", border_style="green"))
